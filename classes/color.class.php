...
class Color
{
    ...
    public $r = 0x00, $g = 0x00, $b = 0x00;

    public function __construct($color)
    {
        if (preg_match('/#?[a-f0-9]{1,2}/i', $color)) {
            $color = ltrim($color, '#');
            list($this->r, $this->g, $this->b) = array_map('hexdec', str_split($color, strlen($color) / 3));
        } else {
            $color = $this->name2hex($color);
            if ($color === false) {
     ...
     * @return array|boolean array with 'red', 'green' and 'blue' elements or false
     */
    public static function name2hex($color_name)
    {
        return array(
                ...
                'yellowgreen' => array('red' => 0x9A, 'green' => 0xCD, 'blue' => 0x32))[$color_name] ?? false;
    }
