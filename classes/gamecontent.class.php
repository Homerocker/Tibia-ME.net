...
class GameContent {
    ...
    public function sync($cat) {
        ...
        $this->xls = PHPExcel_IOFactory::createReader(PHPExcel_IOFactory::identify(__DIR__ . '/../gamecontent_xls/'
                                . $this->sync_files[$cat]['filename']));
        $this->xls = $this->xls->load(__DIR__ . '/../gamecontent_xls/'
                . $this->sync_files[$cat]['filename']);
        $this->xls->setActiveSheetIndex($this->sync_files[$cat]['sheet_index']);
        $cols = PHPExcel_Cell::columnIndexFromString($this->xls->getActiveSheet()->getHighestColumn());
        $rows = $this->xls->getActiveSheet()->getHighestRow();
        for ($row = $this->sync_files[$cat]['start_row']; $row <= $rows; ++$row) {
            if ($cat == 'spells' || $cat == 'food') {
                $cell = $this->xls->getActiveSheet()->getCellByColumnAndRow(0,
                        $row);
            ...
            for ($col = 0; $col <= $cols; ++$col) {
                ...
                if ($cat === 'pets') {
                    ...
                    if ($sync_cell_type['name'] == 'name') {
                        ...
                        $pets_stats_count[$i] = $cell->getRowspan();
                    }
                }
    ...
    private function parse_cell($cell, $filter) {
        $value = $cell->getValue();
        if ($filter == 'intval') {
            if ($cell->getStyle()->getNumberFormat()->getFormatCode() == PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE) {
                $value *= 100;
            }
        } else {
            $value = trim($value);
    ...
    private function get_monster_extra_data($monster_name) {
        $xls = PHPExcel_IOFactory::createReader(PHPExcel_IOFactory::identify(__DIR__ . '/../gamecontent_xls/Monsters.xls'));
    ...
    private function parse_icon($cat, $row, $col) {
        ...
        foreach ($this->DrawingCollection as $drawing) {
            ...
            $im = is_a($drawing, 'PHPExcel_Worksheet_MemoryDrawing') ? $drawing->getImageResource()
                        : Images::imagecreate($drawing->getPath());
            if (!empty($icons) && imagesx($icons[0]) * imagesy($icons[0]) < imagesx($im)
                    * imagesy($im)) {
                array_unshift($icons, $im);
            } else {
                $icons[] = $im;
            }
            imagedestroy($im);
